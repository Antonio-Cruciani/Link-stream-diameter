...
def all_pairs_reachable(graph):
    eat_dist = td.EarliestArrivalPath()
    all_pairs = 0
    for v in range(graph.get_num_nodes()):
        eat_dist.compute_distances(graph=graph, start_node=v)
        distances = eat_dist.get_distances()
...
def ifub_on_landmarks(graph, landmarks=None):
    ...
    fp_dist = td.FastestPathOnePass()

    # Create opposite graph to compute backward fastest paths distances
    g_path_op = fp_dist.gen_opposite(folder=graph.get_file_path().rsplit('/', 1)[0] + '/',
                                     file=graph.get_file_path().rsplit('/', 1)[1])

    g_op = tg.Graph(file_path=g_path_op, is_directed=graph.get_is_directed(), latest_node=graph.get_latest_node())

    # Get greatest distances fw (EAT) and bw (LDT) respectively from and to landmarks
    # Choose the upper bound like the higher upper bound from all landmark's upper bounds
    ub = -1
    idx_landmark = -1
    for i in range(num_landmark):
        if (distances_eat[i][0] - distances_ldt[i][0]) > ub:
            ub = distances_eat[i][0] - distances_ldt[i][0]
            idx_landmark = i
    lb = 0

    while ub > lb:
        index_node_bw = ind_ldt_node[idx_landmark][indices_bw[idx_landmark]]
        fp_dist.compute_distances(graph=graph, start_node=index_node_bw)
        num_visits += 1
        ...
        _, ecc_fw = eccentricity_reachable(fp_dist.get_distances(), is_inA_B=reachables_nodes)
        ...
        fp_dist.compute_distances(graph=g_op, start_node=index_node_fw)
        num_visits += 1
        ...
        _, ecc_bw = eccentricity_reachable(fp_dist.get_distances(), is_inA_B=reachables_nodes)
        ...
        for i in range(num_landmark):
            while indices_fw[i] < len(distances_eat[i]) and not to_be_considered_fw[ind_eat_node[i][indices_fw[i]]]:
                indices_fw[i] += 1
            while indices_bw[i] < len(distances_ldt[i]) and not to_be_considered_bw[ind_ldt_node[i][indices_bw[i]]]:
                indices_bw[i] += 1

            if indices_fw[i] < len(distances_eat[i]) and indices_bw[i] < len(distances_ldt[i]):
                if (distances_eat[i][indices_fw[i]] - distances_ldt[i][indices_bw[i]]) > max_distance:
                    max_distance = distances_eat[i][indices_fw[i]] - distances_ldt[i][indices_bw[i]]
                    idx_landmark = i
