...
def ifub_on_landmarks(graph, landmarks=None):
    ...
    # Get smallest distance bw (LDT) to landmarks
    # Choose the upper bound like the higher upper bound from all landmark's upper bounds (smallest LDT distance)
    ub = np.inf
    idx_landmark = -1
    for i in range(num_landmark):
        if distances_ldt[i][0] < ub:
            ub = distances_ldt[i][0]
            idx_landmark = i
    ...
    while ub < lb:
        ...
        back_bfs = Bbfs.TemporalBackwardBFS(graph=g_op, start_node=index_node_bw)
        back_bfs.bfs()
        num_visits += 1
        ...
        _, ecc_fw = eccentricity_reachable(back_bfs.get_eat(), is_inA_B=reachables_nodes)
        ecc_fw = -ecc_fw
        ...
        for i in range(num_landmark):
            ...
            if indices_bw[i] < len(distances_ldt[i]):
                if (distances_ldt[i][indices_bw[i]]) < max_distance:
                    max_distance = distances_ldt[i][indices_bw[i]]
                    idx_landmark = i

        if max_distance == np.inf:  # All nodes have been examined
            return lb, num_visits, num_distinct_A, num_distinct_B, pairs_number
